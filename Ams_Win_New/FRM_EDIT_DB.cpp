...
//    OP->DBStr
    ...
    if (PrevFlags&eoNewPos)
    {
        ...
        ODS->Insert();
        ODS->FieldByName("CATID")->AsInteger = FrmBrowsDb->GetCurrDbCat();
    ...
    if (!(PrevFlags&eoNewPos))
    {
        ...
        if (VerInp && ArtInp)
        {
            if ( /*CHLIGA (!(PrevFlags & eoHideVersion)) && EO CHLIGA*/
                (
 ...
 //----------------CH LIGA
            if (!(PrevFlags & eoNewPos)) {
               if ((FO.RealName.UpperCase() != "CATID") && (FO.RealName.UpperCase() != "VER") &&  (FO.RealName.UpperCase() != "ARTICLE") &&
               ...
               if ((FO.RealName.UpperCase() != "CATID"))
               IS->SetValToFld(ODS->Fields->Fields[FO.FldNo]);
            }
 //----------------EO CH LIGA
        }
    ...
    if (FormStyle==fsNormal) Hide();
    ModalResult = 1;
    if ((PrevFlags&eoNewPos) && (FrmBrowsDb->Name=="FrmBrowsAn")) ((TFrmBrowsAn*)FrmBrowsDb)->ActEditAnExecute(this);
}
//---------------------------------------------------------------------------
...
//---------------------------------CH LIGA
    ...
    try
    {
      if (!FrmBrowsDb->WithoutVer) {
        ...
        OQ->DeleteVariables();
        OQN->DeleteVariables();
        OQN->SQL->Text= "select Catid, Ver from "+BaseUserName+"."+FldsDescr->GetDbName()+
                        " where Article='"+ODS->FieldByName("ARTICLE")->AsString+"'";
        OQN->Execute();
        while (!OQN->Eof) {
           OQ->SQL->Clear();
           OQ->SQL->Text="insert into "+BaseUserName+"."+FldsDescr->GetDbName()+"( CATID, ARTICLE, VER) values ('"+
           ...
           OQ->SQL->Text="update "+BaseUserName+"."+FldsDescr->GetAnName()+
                         ...
                         OQN->FieldAsString(1)+"' where AARTICLE='"+
